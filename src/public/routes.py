...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        print(f"\n{user}\n")
        if user:
            token = token_manager.sign(f'{user.role}{user.email}{user.id}').decode('utf-8')
            user.token = token
            db.session.commit()
    ...
    return render_template('public/reset_request.html', form=form, title='Reset Password')
...
def reset_password(username, token):
    ...
    if request.method == 'GET':
        if user and user.validate_token(token):
            return render_template('public/reset_password.html', form=form, title='Change Password')
        else:
